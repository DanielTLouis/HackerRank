...
determines the difference between a number and its reverse. For instance, given the
number 12, its reverse is 21. Their difference is 9. The number 120 reversed is 21,
...
in the range that are beautiful. Beautiful numbers are defined as numbers where
|i-reverse(i)| is evenly divisible by k. If a day's value is a beautiful number, it is
a beautiful day. Return the number of beautiful days in the range.
...
Explanation
    ...
    |23-32|/6=9/6=1.5
    Only two days, 20 and 22, in this interval are beautiful. Thus, we print 2 as our
    answer.
"""
...
def beautifulDays(i, j, k):
    ...
    for d in range(i,j+1):
        day = abs(d-(int(str(d)[::-1])))#day - inverse day (convert to string reverse)
        if(float(day//k) == float(day/k)):#?intDivision == floatDivision : wholeNumber
            beautyDays += 1
    return beautyDays
