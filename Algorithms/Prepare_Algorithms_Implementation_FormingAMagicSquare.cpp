We define a magic square* to be an nXn matrix of distinct positive integers from 1 to
n^2 where the sum of any row, column, or diagonal of length  is always equal to the
same number: the magic constant.
You will be given a 3X3 matrix s of integers in the inclusive range [1,9]. We can
...
Sample Output 0
    1
Explanation 0
    If we change the bottom right value, s[2][2], from 5 to 6 at a cost of |6-5|-1, s
...
int formingMagicSquare(vector<vector<int>> s) {
   vector<vector<vector<int>>> possiblities = {
       {{8,1,6},{3,5,7},{4,9,2}},
       {{6,1,8},{7,5,3},{2,9,4}},
       {{4,9,2},{3,5,7},{8,1,6}},
       {{2,9,4},{7,5,3},{6,1,8}},
    ...
    for(int square = 0; square < possiblities.size(); square++)
    {
        int cost = 0;
        for(int i = 0; i < 3; i++)
        {
            for(int j = 0; j < 3; j++)
            {
                cost += abs(possiblities[square][i][j]-s[i][j]);
            }
        }
    ...
    for(int i = 0; i < possible_costs.size(); i++){
        if(possible_costs[i] < ans)
        {
            ans = possible_costs[i];
        }
    }
    return ans;
}
