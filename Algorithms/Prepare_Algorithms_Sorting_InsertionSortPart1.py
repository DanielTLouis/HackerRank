...
Insertion Sort
    ...
    algorithm. We will first start with a nearly sorted list.
Insert element into sorted list
    ...
    performing the insertion. It will instead demonstrate the brute-force method 
    in detail.
    Assume you are given the array arr=[1,2,4,5,3] indexed 0...4. Store the value 
    of arr[4]. Now test lower index values successively from 3 to 0 until you 
    reach a value that is lower than arr[4], at arr[1] in this case. Each time 
    your test fails, copy the value at the lower index to the current index and 
    print your array. When the next lower indexed value is smaller than arr[4], 
...
Example
    ...
    each element to the left until a smaller value is reached. Here are the 
...
Returns
    ...
    is expected.
Input Format
    The first line contains the integer , the size of the array arr.
    The next line contains  space-separated integers arr[0]...arr[n-1].
Constraints
    1<=n<=1000
    -10000<=arr[i]<=10000
Output Format
    Print the array as a row of space-separated integers each time there is a 
    shift or insertion.
Sample Input
    5
    2 4 6 8 3
