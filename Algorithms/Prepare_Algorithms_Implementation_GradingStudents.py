...
Examples
    ...
    Given the initial value of grade for each of Sam's  students, write code to
...
Explanation 0
    image
    1.Student 1 received a 73, and the next multiple of 5 from 73 is 75. Since 75-73=2,
    ...
    4.Student 4 received a grade below 33, so the grade will not be modified and the
    student's final grade is 33.
"""
#!/bin/python3
...
def gradingStudents(grades):
    ...
    for i in grades:
        ...
        ones = int(str(i)[1])
        tens = int(str(i)[0])
        if(ones > 2 and ones < 6):
            ones = 5
            newgrads.append(int(str(tens) + str(ones)))
        elif(ones > 7):
            ones = 0
            tens += 1
            newgrads.append(int(str(tens) + str(ones)))
