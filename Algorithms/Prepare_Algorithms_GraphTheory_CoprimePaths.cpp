/*
You are given an undirected, connected graph, G, with n nodes and m edges where m=n-1. Each
node i is initially assigned a value, nodei, that has at most 3 prime divisors.
You must answer q queries in the form u v. For each query, find and print the number of (x,y)
pairs of nodes on the path between u and v such that gcd(nodex, nodey) = 1 and the length of
the path between u and v is minimal among all paths from u to v.
Input Format
    ...
    describing an edge between nodes u and v.
    Each of the q subsequent lines contains two space-separated integers, u and v, describing
    a query.
Constraints
    1<=n,q<=25X10^3
    1<=nodei<=10^7
    1<=u,v<=n
Output Format
    For each query, print an integer on a new line denoting the number of (x ,y) pairs of
    nodes on the path between u and v such that gcd(nodex, nodey) = 1 and the length of the
    path between u and v is minimal among all paths from u to v.
Sample Input 0
    6 5
    3 2 4 1 6 5
    1 2
...
Sample Output 0
    ...
    3
Explanation 0
    The diagram below depicts graph G and the u<->v paths specified by each query, as well as
    the Pair Values for each path in the form (nodex, nodey):
        image
    Recall that, for each queried path, we want to find and print the number of (x,y) pairs of
    nodes such that gcd(anodex,nodey)=1.
*/
