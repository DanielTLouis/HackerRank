...
any integer y such that p(p(y))=x and keep a history of the values of y in a return
array.
Example
    p=[5,2,1,3,4]
    Each value of x between 1 and 5, the length of the sequence, is analyzed as
    follows:
    1.x=1=p[3],p[4]=3, so p[p[4]]=1
    2.x=2=p[2], p[2]=2, so p[p[2]]
    3.x=3=p[4],p[5]=4, so p[p[3]]
...
def permutationEquation(p):
    ...
    y = []
    p = [x - 1 for x in p]#need to zero index the array (idk why it isnt zero indexed)
    for i in range(0,len(p)):
        for j in range(0, len(p)):
            if(p[p[j]]==i):
                y.append(j+1)#add one to get back to original position instead of zero
                break
    return y
