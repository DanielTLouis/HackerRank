...
Example
    ...
    ["betta","alpha"]. Ordered alphabetically, the names are
    printed as:
        alpha
        beta
Input Format
...
Constraints
    2<=N<=5
    There will always be one or more students having the second
    lowest grade.
Output Format
    Print the name(s) of any student(s) having the second lowest
    grade in. If there are multiple students, order their names
...
Explanation 0
    ...
    The lowest grade of 37.2 belongs to Tina. The second lowest
    grade of 37.21 belongs to both Harry and Berry, so we order
...
if __name__ == '__main__':
    ...
    students.sort()
    students.sort(key=myFunct)
    lowest = students[0][1]
    secondLowest = -1
    for i in range(0,len(students)):
        if(students[i][1] != lowest and secondLowest == -1):
            secondLowest = students[i][1]
            print(students[i][0])
            continue
        elif(students[i][1] == secondLowest):
            print(students[i][0])
            continue 
