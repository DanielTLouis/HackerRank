...
where p denotes the ASCII* value of the ith character in string p, a=100001, and m=10^9 +7.
Nikita has a string, s, consisting of n lowercase letters that she wants to perform q queries
on. Each query consists of an integer, k, and you have to find the value of f(wk) where wk is
the kth alphabetically smallest palindromic substring** of s. If wk doesn't exist, print -1
instead.
* http://ascii.cl/ *
** https://en.wikipedia.org/wiki/Lexicographical_order **
Input Format
    The first line contains 2 space-separated integers describing the respective values of
    n (the length of string s) and q (the number of queries).
    The second line contains a single string denoting s.
    Each of the q subsequent lines contains a single integer denoting the value of k for
    a query.
Constraints
    1<=n,q<=10^5
    1<=k<=(n(n+1))/2
    It is guaranteed that string s consists of lowercase English alphabetic letters only
        (i.e., a to z).
    a=10^5 +1
    m=10^9 +7.
Scoring
    1<=n,q<=10^3 for 25% of the test cases.
    1<=n,q<=10^5 for 100% of the test cases.
Output Format
    For each query, print the value of function f(wk) where wk is the kth alphabetically
    smallest palindromic substring of s; if wk doesn't exist, print -1 instead.
Sample Input
    5 7
    abcba
    1
...
Sample Output
    ...
    -1
Explanation
    There are 7 palindromic substrings of "abcba". Let's list them in lexicographical order
    and find value of wk:
        1. w1="a", f(w1)=97
        2. w2="a", f(w2)=97
        3. w3="abcba", f(w3)=696207567
        4. w4="b", f(w4)=98
        5. w5="b", f(w5)=98
        6. w6="bcb", f(w6)=29493435
        7. w7="c", f(w7)=99
        8. doesn't exist, so we print -1 for k=8.
*/
#include <bits/stdc++.h>
using namespace std ;
