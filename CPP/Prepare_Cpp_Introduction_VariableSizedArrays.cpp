...
For each query, find and print the value of element a[i] in the array at location 
a[i] on a new line.
...
Input Format
    ...
    the format k a[i]0 a[i]1 … a[i]k-1 describing the k-element array located at 
    a[i].
    Each of the q subsequent lines contains two space-separated integers 
    describing the respective values of i (an index in array a) and j (an index 
    in the array referenced by a[i]) for a query.
Constraints
    1<=n<=10^5
    1<=q<=10^5
    N<=Sumationof(K)<=3*10^5
    0<=i<n
    0<=j>k
    All indices in this challenge are zero-based.
    All the given numbers are non negative and are not greater than 10^6
Output Format
    For each pair of i and j values (i.e., for each query), print a single 
    integer that denotes the element located at index j of the array referenced 
    by a[i]. There should be a total of q lines of output.
Sample Input
    2 2
    3 1 5 4
    5 1 2 8 9 3
...
Explanation
    ...
    We perform the following  queries:
        ...
        We must print the value at index j=1 of this array which, as you can see, 
        is 5.
        2.Find the array located at index i=1, which corresponds to 
        a[1]=[1,2,8,9,3]. We must print the value at index j=3 of this array 
...
int main() {
    ...
    int n,col, i,j,findrow,findcol,find;
    cin>>n>>find;
    int *arr[n];
    for(i=0;i<n;i++)
    {
